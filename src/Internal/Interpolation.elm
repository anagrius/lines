...
toCommands interpolation =
    case interpolation of
        ...
        SteppedBefore ->
            stepped (Just 10000000) before

        SteppedAfter ->
            stepped (Just 10000000) after
...
stepped : Maybe Float -> (Point -> Point -> List Point) -> List (List Point) -> List (List Command)
stepped bucketSize step sections =
    let
        lastPoint last =
            case bucketSize of
                Just size ->
                    [ { x = last.x + size, y = last.y } ]
